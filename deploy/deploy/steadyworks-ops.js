...
// Every job is subcontracted out — margin isn't take-home, it's what's left
...
function view_pipeline(){
    ...
    return `<div class="kanban-col ${muted?'lost-col':''} ${divider?'job-divider':''}" data-stage="${stage.id}" ondragover="event.preventDefault();this.classList.add('drag-over')" ondragleave="this.classList.remove('drag-over')" ondrop="dropPaint(event,'${stage.id}')">
      ...
      ${items.map(r=>{
        ...
        return `<div class="kanban-card" draggable="true" ondragstart="dragPaint(event,'${r.id}')" onclick="openPaintRecordModal('${r.id}')" title="${esc(r.clientName||'Unnamed')}">
          <div class="kc-name">${esc(r.clientName||'Unnamed')}</div>
  ...
  <div class="grid grid-4 mt-10">
    ...
    <div class="card kpi-card"><div class="kpi-label">Joint Profit (this wk)</div><div class="kpi-value">${fmt(thisWeekProfit)}</div><div class="kpi-delta ${thisWeekProfit>=s.weeklyProfitTarget?'up':'down'}">Target ${fmt(s.weeklyProfitTarget)}</div></div>
...
function refreshPaintCalcSummary(){
  ...
  const reinvestment = margin * (Number(ps.reinvestmentPct)||0)/100;
  ...
  el.innerHTML = `
    ...
    <div><div class="pcs-label">Reinvestment (${ps.reinvestmentPct||0}%)</div><div class="pcs-value">${fmt(reinvestment)}</div></div>
    <div><div class="pcs-label">SteadyWorks Share</div><div class="pcs-value" style="color:#22C55E;">${fmt(swShare)}</div></div>
    <div><div class="pcs-label">Fabs Share</div><div class="pcs-value">${fmt(fabsShare)}</div></div>
    <div><div class="pcs-label">Split</div><div class="pcs-value">${swPct}% / ${100-swPct}%</div></div>
  `;
}
